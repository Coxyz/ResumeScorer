...
const STRONG_ACTION_VERBS = new Set([
	...
	'unified',
	'upgraded',
	// FR localization: French CV bullets conventionally lead with a past
	// participle used adjectivally ("Développé...", "Conçu...") rather than
	// an imperative infinitive. Added so a French bullet gets the same
	// fair shot at this check as an English one.
	'conçu',
	'concu',
	'développé',
	'developpe',
...
export function scoreExperience(bullets: string[]): ExperienceScore {
	...
	for (const bullet of bullets) {
		...
		const firstWord = bullet
			...
			.split(/\s+/)[0]
			?.toLowerCase()
			.replace(/[^a-zà-ÿ]/g, ''); // FR fix: keep accented letters (was stripping é/è/à etc.)
