...
const DEGREE_LEVELS: Record<string, number> = {
	...
	certificate: 1,
	certification: 1,
	// FR localization: French degree/level vocabulary. RNCP levels 6/7 are the
	// official French equivalence to bachelor's/master's (France Competences),
	// so they're mapped accordingly rather than left undetected.
	doctorat: 5,
	master: 4,
	'master 2': 4,
	'master 1': 4,
...
export function scoreEducation(educationText: string): EducationScore {
	...
	// check for degree mention. word-boundary regex (not plain .includes) to
	// avoid false hits from short ambiguous abbreviations landing inside an
	// unrelated word (e.g. the key "ma" matching inside "majoritairement").
	// escape regex metacharacters present in keys like "ph.d", "b.s", "bac+5".
	const escapeRegex = (s: string) => s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
	let highestDegree = 0;
	let degreeFound = '';
	for (const [degree, level] of Object.entries(DEGREE_LEVELS)) {
		const pattern = new RegExp(`(?:^|[^a-z0-9])${escapeRegex(degree)}(?:$|[^a-z0-9])`, 'i');
		if (pattern.test(lowerText) && level >= highestDegree) {
			highestDegree = level;
			degreeFound = degree;
		}
	}
	...
	const fieldIndicators = /\b(?:in|of|en|d')\s+[A-Za-zÀ-ÿ]/;
	const hasField =
		...
		// FR localization
		/(?:informatique|ing[ée]nierie|architecture\s*informatique|syst[èe]mes?\s*embarqu[ée]s?|[ée]lectronique|r[ée]seaux?|math[ée]matiques?|physique|[ée]conomie|gestion|marketing|droit)/i.test(
			educationText
		);
	if (hasField) {
		score += 15;
