...
// FR localization: French CVs mark ongoing roles/formations with
// "aujourd'hui", "en cours", "actuellement", "présent" instead of
// "present"/"current". Without these a real French date range like
// "01/2026 - aujourd'hui" is invisible to the date extractor.
const CURRENT_INDICATORS =
	/\b(present|current|now|ongoing|today|aujourd.?hui|en\s*cours|actuel(?:lement)?|pr[ée]sent)\b/i;
const CURRENT_ALTERNATION = "present|current|now|ongoing|today|aujourd.?hui|en\\s*cours|actuel(?:lement)?|pr[ée]sent";

// "Jan 2023 - Present", "January 2023 - December 2024", "01/2023 - 12/2024"
const DATE_RANGE_PATTERNS = [
	// Month Year - Month Year (or Present)
	new RegExp(
		`(?:(?:jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:t(?:ember)?)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)\\s*\\.?\\s*\\d{4})\\s*[-–—~to]+\\s*(?:(?:jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:t(?:ember)?)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)\\s*\\.?\\s*\\d{4}|${CURRENT_ALTERNATION})`,
		'gi'
	),

	// MM/YYYY - MM/YYYY (or Present / aujourd'hui / en cours)
	new RegExp(`\\d{1,2}\\/\\d{4}\\s*[-–—~to]+\\s*(?:\\d{1,2}\\/\\d{4}|${CURRENT_ALTERNATION})`, 'gi'),

	// Year - Year (or Present / aujourd'hui / en cours)
	new RegExp(
		`\\b(20\\d{2}|19\\d{2})\\s*[-–—~to]+\\s*(?:(20\\d{2}|19\\d{2})|${CURRENT_ALTERNATION})\\b`,
		'gi'
	),
